...
| 26 | the +324% AV ablation shows the shared cortical dynamics are load-bearing, so the programme's central bet is standing | the ablation's OWN artefact, `out/ablation_video_multifilm.json`, carries a persistence baseline of 0.0476 and a `skill_vs_persistence` for every arm -- and **every arm loses to persistence**: full **−1.08**, frozen −4.42, no_assoc −4.40, bypass **−8.17**. The best of them loses to copying the previous frame by a factor of **2.08**. So +324% is a difference between models that all lose to a trivial baseline, and "load-bearing" holds only in the sense that removing the dynamics makes a losing model lose considerably more. `docs/TRAINING.md` reported the loss column and dropped the skill column that sat beside it in the same file | reading the artefact completely instead of the table built from it -- no new measurement, the numbers had been there since it was written |
| 25 | the contrastive paired arm's win over the ridge shows the cortical dynamics carrying stimulus-to-MEG structure | the arm is real -- **4.38x chance against the ridge's 2.89x**, paired CI [+1.15, +1.82], with a shuffled-pairing control at **1.02x** -- but the BYPASS arm, dynamics removed and nothing else changed, reaches **4.53x** and runs **174x faster** (35 s against 6,087 s). The sheet is not carrying it. **AMENDED at three seeds:** the "−0.16x, CI [−0.31, −0.01], significantly better" reading was from ONE run of each on a CI that resampled evaluation windows only; on the pre-registered multi-seed rule the arms are **NOT DISTINGUISHABLE** (means 4.220 against 4.397, difference −0.177x inside a 0.290x seed spread), and a paired test pre-registered on FRESH seeds 3-5 gives 95% CI **[−0.365, +0.112], including zero** -- the tight −0.15/−0.17/−0.21 clustering on seeds 0-2 did not replicate (−0.23/−0.04/−0.11). The standing claim is *the cortex is unnecessary here and costs nothing measurable*, not *the cortex is harmful* | running the bypass arm, then running three seeds of each when the single-seed CI was noticed to contain no seed variance at all |
...
the mean of the per-seed paired differences with its own standard error across seeds, on those
three seeds alone. Seeds 0-2 were deliberately not pooled in, because they are what suggested the
test.

| seeds | paired differences (intact − bypass) | mean | sd | t(2) |
|---|---|---:|---:|---:|
| 0, 1, 2 — *what suggested the test* | −0.15, −0.17, −0.21 | −0.177 | 0.031 | **−10.0** |
| **3, 4, 5 — *what judges it*** | **−0.23, −0.04, −0.11** | **−0.127** | 0.096 | **−2.28** |

**95% CI [−0.365, +0.112] — INCLUDES ZERO.** 

**PREDICTED: "the paired difference stays small, negative, and its interval excludes zero."
REFUTED.** I said the cortex would turn out to cost about 0.18x of chance, reliably. It does not.

**And the reason is exactly why fresh seeds were demanded.** The differences on seeds 0-2 were
−0.15, −0.17, −0.21 — a spread of 0.06 and a paired `t` of −10. On seeds 3-5 they are −0.23,
−0.04, −0.11 — a spread of 0.19, three times wider. **The tight clustering that made the effect
look certain was itself luck.** Had I pooled all six, or promoted the post-hoc paired analysis
when I first noticed it, this would have been published as a real effect.

**THE SETTLED CLAIM.** For stimulus-to-MEG retrieval on LibriBrain, the cortical dynamics are
**unnecessary and cost nothing measurable**. Not "unnecessary and slightly harmful" — the cost is
indistinguishable from zero across six seeds under two pre-registered rules, the unpaired one on
seeds 0-2 and the paired one on seeds 3-5.

*Descriptively, and not the pre-registered statistic:* bypass is at least as good as intact in
**6 of 6** seeds. A sign test on that is p = 0.016. It is recorded because it is true and because
hiding it would be its own dishonesty, but it does not overturn the interval above, it is a
second post-hoc test of the same six numbers, and it does not change what the arms are worth: the
magnitude is indistinguishable from zero and a difference too small to measure is too small to
matter.

**What stands, across all six seeds and unaffected by any of this:** the contrastive objective
beats the linear ridge (intact 4.09-4.38x, bypass 4.27-4.53x, ridge 2.89x, shuffled control
1.02x), and the sheet earns none of it while costing **174x** the compute.

---


## 2026-09-12 -- VERDICT at three seeds: NOT DISTINGUISHABLE. The prediction held, and the rule has a flaw worth naming.

Three seeds of each arm, each scored exactly on all 2,820 held-out windows:

| arm | seed 0 | seed 1 | seed 2 | mean | range |
|---|---:|---:|---:|---:|---:|
| contrastive intact | 4.38x | 4.19x | 4.09x | **4.220x** | 0.290 |
| contrastive BYPASS | 4.53x | 4.36x | 4.30x | **4.397x** | 0.230 |

**The pre-registered rule** -- compare multi-seed means, uncertainty the larger of the two arms'
seed spreads -- gives `|−0.177| < 0.290` → **NOT DISTINGUISHABLE.**

**Predicted before the seeds ran: "the −0.16x does not survive." It did not.** Ledger row 25 is
amended: the single-seed "significantly better" came from a CI that resampled evaluation windows
and contained no seed variance at all. The standing claim is **the cortex is unnecessary here**,
not that it is harmful.

**And the thing I also predicted would survive, did.** *"Two arms landing in the same place is not
something seed noise can manufacture."* Bypass is at least as good as intact in **3 of 3 seeds**,
at 174x less compute. That is the durable finding.

### The flaw in my own rule, named rather than exploited

The per-seed differences are **−0.15, −0.17, −0.21** — all negative, tightly clustered, paired
`t(2) = −10.0`. A **paired** analysis would call that significant. The rule I committed is
**unpaired**, and this design is naturally paired: intact and bypass at seed S share
`torch.manual_seed(S)`, so the same initialisation and the same topology draw.

**I am not switching to the analysis that gives significance after seeing the data.** That is the
whole reason the rule was fixed in advance, and row 9 of the ledger exists because someone once
did exactly that. The pre-registered verdict stands: NOT DISTINGUISHABLE.

What the paired structure earns is a *new* pre-registration, judged on *new* seeds. Fixed now:
**seeds 3, 4 and 5 of both arms; the statistic is the mean of the per-seed paired differences with
its own standard error across seeds; the difference is real if its 95% interval excludes zero on
those three seeds alone.** Seeds 0–2 are not pooled in, because they are what suggested the test.

Note the pairing is only partial: bypass constructs the same `CorticalDynamics` (and so draws the
same topology from the global RNG) but never uses it, so the two arms' RNG trajectories diverge
after construction. The paired statistic is therefore weaker than a fully matched one, and that is
stated here rather than discovered later.

**PREDICTED for seeds 3-5:** the paired difference remains small and negative and its interval
excludes zero -- i.e. the cortex costs about 0.18x of chance, reliably, which is 4% of the effect
the objective change buys. If the interval includes zero, the honest reading returns to
*indistinguishable* and stays there.
...
**WITHDRAWN — ledger row 29.** Everything in this section below rests on a run
whose TARGET WAS NEVER NORMALISED, and it does not survive that. The paragraph
that follows is kept because the reasoning was sound given a number that was not.
